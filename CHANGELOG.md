...
- 单副牌新增“满贯机会”，同时比较 DD 可成的最高满贯、Par 满贯和实战到达率。
- 评语会识别“低 HCP 但牌型、双配合与缺门带来满贯潜力”的牌，不再只列最低成局定约。
- 当场上主流无将而套约可做满贯时，明确提示无将与套约的双明上限差异。
- 修复未加倍定约被提问文案误写成“被加倍”，并避免重复显示同一牺牲定约。
- 对未加倍的疑似牺牲结果增加证据边界：没有叫牌记录时，不反推叫牌者意图，并提醒可能是对方未处罚到位。
